...
int main(int argc, char **argv) {
  ...
  Kokkos::initialize(argc, argv);{
    ...
    img.modify_device();
    Kokkos::parallel_for("get_color", WIDTH*HEIGHT, KOKKOS_LAMBDA(const int& i) {
        int x = i % WIDTH;
        int y = i / WIDTH;
        unsigned int v = i;
        ...
        for (int p = samplesCount; p--;) {
          ...
          color = color + Trace(position, !((goal+rand_left) + left * ((x+randomVal(v)) - WIDTH / 2.0f + randomVal(v)) + up * ((y+randomVal(v)) - HEIGHT / 2.0f + randomVal
                                                                                                                               (v))), v);
        }
