...
KOKKOS_INLINE_FUNCTION int RayMarching(const Vec& origin, const Vec& direction, Vec& hitPos, Vec& hitNorm) {
  ...
  for (float total_d = 0; total_d < 100; total_d += d) {
    if ((d = QueryDatabase(hitPos = origin + direction * total_d, hitType)) < .01 || ++noHitCount > 99) {
      hitNorm.x = QueryDatabase(hitPos + Vec(.01, 0), noHitCount) - d;
      hitNorm.y = QueryDatabase(hitPos + Vec(0, .01), noHitCount) - d;
      hitNorm.z = QueryDatabase(hitPos + Vec(0, 0, .01), noHitCount) - d;
      return !hitNorm, hitType;
    }
  }
  return 0;
}
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    Kokkos::parallel_for(WIDTH * HEIGHT, KOKKOS_LAMBDA(const unsigned int i) {
        ...
        for (unsigned int p = samplesCount; p--;) {
          ...
          color = color + Trace(position, !((goal+rand_left) + left * ((x+randomVal(v)) - WIDTH / 2.0f + randomVal(v)) + up * ((y+randomVal(v)) -
                                                                                                                               HEIGHT / 2.0f + randomVal
                                                                                                                               (v))), v);
        }
        ...
        unsigned int indx = (y * WIDTH * 3) + (x * 3);
        img[indx]   = color.x;
        img[indx+1] = color.y;
        img[indx+2] = color.z;
      });
    ...
    std::ofstream myfile;
    myfile.open ("example2.ppm");
    myfile << "P6 " << WIDTH << " " << HEIGHT << " 255 ";
    for (int y = HEIGHT; y--;) {
      for (int x = WIDTH; x--;) {
        unsigned int indx = (y * WIDTH * 3) + (x * 3);
        myfile << img[indx] << img[indx+1] << img[indx+2];
      }
    }
  }
  Kokkos::finalize();
